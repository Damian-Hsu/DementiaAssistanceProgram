...
from services.ComputeServer.DTO import *
import cv2
import numpy as np
...
@timer
def get_video_frames(video_uri: str, target_fps: int = 3):
    video_info = {
                "video_url": video_uri,
                "fps": None,
...
@timer
def get_video_frames_fast(video_uri: str, target_fps: int = 3):
    """
    GPT改我程式的加速版
    """
    if target_fps <= 0:
...
def _make_success_jobresult(job: dict,
    ...
    video_start_dt = _parse_iso_dt(job.get("video_start_time"))
    video_end_dt = None
    try:
...
def _make_failed_jobresult(job: dict,
    ...
    return {
        ...
        "duration": float(video_info["duration"]) if (video_info and video_info.get("duration") is not None) else None,
        "metrics": metrics,
        "events": [],
    }


def video_description_extraction(job: dict):
    """
    step 1 : 從 job 取得 video_uri
...
if __name__ == "__main__":
    ...
    job = {
        "job_id": "test_job",
        "video_start_time": "2025-01-01T00:00:00Z",
        "type": "video_description_extraction",
        "trace_id": "test_trace",
        "segment_uri": "datas/test/777402600.292068.mp4",
        "params": {
            "target_fps": 2,
            "blur_threshold": 60.0,
    ...
    result = video_description_extraction(job)
